...
export function AvailabilityCalendar({ adminMode = false, blocks, onBlocksChange }: AvailabilityCalendarProps) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={prevMonth} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors" aria-label="Mois précédent">
          ...
          <button onClick={nextMonth} className="p-2 rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors" aria-label="Mois suivant">
      ...
      {adminMode && (
        <div className="mt-6 border-t border-slate-200 pt-4">
          {!showAddBlock ? (
            <button onClick={() => setShowAddBlock(true)} className="btn-primary text-xs">
            ...
            <div className="space-y-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <input type="date" value={newBlockStart} onChange={(e) => setNewBlockStart(e.target.value)} className="input-field" />
                </div>
                <div>
                  <label className="label-field">Date de fin</label>
                  <input type="date" value={newBlockEnd} onChange={(e) => setNewBlockEnd(e.target.value)} className="input-field" />
                </div>
                <div>
                  <label className="label-field">Libellé (optionnel)</label>
                  <input type="text" value={newBlockLabel} onChange={(e) => setNewBlockLabel(e.target.value)} placeholder="Maintenance, Congé..." className="input-field" />
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={handleAddBlock} disabled={saving || !newBlockStart} className="btn-primary text-xs">
                  {saving ? 'Enregistrement...' : 'Confirmer'}
                </button>
                <button onClick={() => setShowAddBlock(false)} className="btn-secondary text-xs">Annuler</button>
              </div>
            </div>
          )}
          ...
          {blocks.length > 0 && (
            <div className="mt-4 space-y-2">
              ...
              {blocks.filter((b) => b.status === 'blocked').map((b) => (
                <div key={b.id} className="flex items-center justify-between p-3 rounded-lg bg-rose-50 border border-rose-200">
                  ...
                  <button onClick={() => handleDeleteBlock(b.id)} className="text-rose-600 hover:text-rose-800 text-sm font-medium">
                    Supprimer
                  </button>
                </div>
              ))}
